                    ...
                    <form action="{{ route('asistencias.store') }}" method="POST">
                        ...
                        <div class="tab-content border border-top-0 p-3" style="max-height:480px;overflow-y:auto;">
                            <div class="tab-pane fade show active" id="tabPresentes" role="tabpanel">
                                <div class="mb-2">
                                    <button type="button" class="btn btn-sm btn-success" onclick="marcar('pres', true)">Marcar todos</button>
                                    <button type="button" class="btn btn-sm btn-outline-secondary" onclick="marcar('pres', false)">Desmarcar todos</button>
                                    <small class="text-muted ml-2"><i class="fas fa-info-circle"></i> Sólo los marcados aquí se registran como presentes.</small>
                                </div>
...
function renderLista(estudiantes, tipo, presentesSet, horas) {
...
$(document).ready(function() {
    ...
    function cargarEstudiantes() {
        ...
        if (!curCodigo) {
            $('#listaPresentes,#listaFaltas').html('<p class="text-muted">Seleccione un curso para ver los estudiantes</p>');
            actualizarConteos();
            return;
        }
        $.get('{{ url("/api/estudiantes-por-curso") }}/' + curCodigo, { fecha: fecha }, function(resp) {
            ...
            $('#listaPresentes').html(renderLista(estudiantes, 'pres', presentesSet, horas));
