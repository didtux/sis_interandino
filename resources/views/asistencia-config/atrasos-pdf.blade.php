    ...
    <table>
        ...
        <tbody>
            @forelse($atrasos as $index => $atraso)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ $atraso->asis_fecha->format('d/m/Y') }}</td>
                    <td>{{ $atraso->asis_hora }}</td>
                    <td style="text-align: left;">{{ strtoupper($atraso->estudiante->est_nombres ?? 'N/A') }} {{ strtoupper($atraso->estudiante->est_apellidos ?? '') }}</td>
                    <td>{{ $atraso->estudiante->padres->first()->pfam_nombres ?? '-' }}</td>
                    <td>{{ $atraso->estudiante->padres->first()->pfam_numeroscelular ?? '-' }}</td>
                    <td>{{ $atraso->estudiante->curso->cur_nombre ?? '-' }}</td>
                </tr>
            @empty
                <tr>
